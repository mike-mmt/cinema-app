...
export default function AddMovieForm() {
	...
	async function sendMovieData(values: valuesType) {
		try {
			const genres = values.genres.split(',').map((genre) => genre.trim());
			const actors = values.actors.split(',').map((actor) => actor.trim());
	...
	return (
		...
		<form onSubmit={formik.handleSubmit} className='form flex flex-col items-center gap-5 mt-5 w-4/5 *:w-3/5'>
      ...
      /> */}
			<FormField field='title' label='Tytuł' type='text' formik={formik} />
			...
			<FormField field='genres' label='Gatunki oddzielone przecinkami' type='text' formik={formik} />
			<FormField field='actors' label='Aktorzy oddzieleni przecinkami' type='text' formik={formik} />
			<div className='flex flex-col items-center mt-8 gap-6 *:w-4/5'>
				<p className='text-center'>Link do plakatu filmu lub plik plakatu:</p>
				<FormField field='photoUrl' label='URL do zdjęcia' type='text' formik={formik} />
				<input
					id='image'
					name='image'
					type='file'
					...
					onChange={(event: FormEvent) => {
						formik.setFieldValue('image', (event.currentTarget as HTMLInputElement).files?.[0] || null);
			...
			<div className='my-6 p-2'>{responseOutput && <p>{responseOutput}</p>}</div>
		</form>
		// </div>
	);
}
